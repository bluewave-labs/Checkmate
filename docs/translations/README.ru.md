...
В этом репозитории — фронтенд и бэкенд Checkmate, open-source и самостоятельно хостящегося инструмента мониторинга, который в реальном времени отслеживает оборудование серверов, доступность, время отклика и инциденты — и показывает всё это в виде понятных графиков. Checkmate периодически проверяет, доступен ли сервер или сайт и работает ли он как надо, и присылает оповещения и отчёты о доступности, простоях и времени отклика отслеживаемых сервисов в реальном времени.

У Checkmate также есть агент [Capture](https://github.com/bluewave-labs/capture), который собирает данные с удалённых серверов. Capture не обязателен для работы Checkmate, но даёт дополнительную информацию о состоянии CPU, RAM, диска и температуры ваших серверов. Capture работает на Linux, Windows, macOS, Raspberry Pi и на любом устройстве, способном запускать Go.

Checkmate прошёл нагрузочное тестирование с 1000+ активных мониторов — без заметных проблем и просадок производительности.
...
Последнюю сборку [Checkmate](https://demo.checkmate.so/) можно посмотреть прямо сейчас.

Логин — demouser@demo.com, пароль — Demouser1!. (Демо-сервер мы периодически обновляем; если он недоступен, напишите нам в канал Discussions.)
...
Также можно использовать [Coolify](https://coolify.io/), [Elestio](https://elest.io/open-source/checkmate), [K8s](./charts/helm/checkmate/INSTALLATION.md), [Sive Host](https://sive.host) (ЮАР), [Cloudzy](https://cloudzy.com/marketplace/checkmate) или [Pikapods](https://www.pikapods.com/), чтобы быстро поднять инстанс Checkmate. Для мониторинга серверной инфраструктуры потребуется агент [Capture](https://github.com/bluewave-labs/capture); инструкции по его установке лежат в его репозитории.
...
Благодаря многочисленным оптимизациям Checkmate работает с очень низким потреблением памяти и требует совсем немного RAM и CPU. Ниже — потребление памяти инстансом Node.js на сервере, который мониторит 323 сервера каждую минуту:
...
Если есть вопросы, предложения или замечания — несколько вариантов на выбор:

- [Discord](https://discord.gg/NAb6H3UTjK) (предпочтительно)
- [GitHub Discussions](https://github.com/bluewave-labs/Checkmate/discussions) (заглядываем туда время от времени)

Спрашивайте смело и делитесь идеями — нам интересно ваше мнение.
