import { buildUptimeDetailsPipeline, buildHardwareDetailsPipeline, buildMonitorSummaryByTeamIdPipeline, buildMonitorsByTeamIdPipeline, buildMonitorsAndSummaryByTeamIdPipeline, buildMonitorsWithChecksByTeamIdPipeline, buildFilteredMonitorsByTeamIdPipeline, } from "./monitorModuleQueries.js";
...
class MonitorModule {
    ...
    calculateGroupStats = (group) => {
        ...
        return {
            ...
            avgResponseTime: checksWithResponseTime.length > 0
    ...
    getMonitorsByTeamId = async ({ limit, type, page, rowsPerPage, filter, field, order, teamId }) => {
        ...
        const filteredMonitors = await this.Monitor.aggregate(buildFilteredMonitorsByTeamIdPipeline({
            matchStage,
            filter,
            page,
            rowsPerPage,
    ...
    getMonitorsWithChecksByTeamId = async ({ limit, type, page, rowsPerPage, filter, field, order, teamId, explain }) => {
        try {
            ...
            if (explain === true) {
                return this.Monitor.aggregate(buildMonitorsWithChecksByTeamIdPipeline({
                    matchStage,
                    filter,
                    page,
                    rowsPerPage,
            ...
            const queryResult = await this.Monitor.aggregate(buildMonitorsWithChecksByTeamIdPipeline({
                matchStage,
                filter,
                page,
                rowsPerPage,
    ...
    pauseMonitor = async ({ monitorId }) => {
        try {
            const monitor = await this.Monitor.findOneAndUpdate({ _id: monitorId }, [
                {
                    $set: {
