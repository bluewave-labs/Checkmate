...
const nameValidation = joi
    ...
    "string.pattern.base": "Names must contain at least 1 letter and may only include letters, currency symbols, spaces, apostrophes, hyphens (-), periods (.), and parentheses ().",
});
...
const getMonitorsByTeamIdQueryValidation = joi.object({
    ...
    type: joi
        .alternatives()
        .try(joi.string().valid("http", "ping", "pagespeed", "docker", "hardware", "port", "game"), joi.array().items(joi.string().valid("http", "ping", "pagespeed", "docker", "hardware", "port", "game"))),
    page: joi.number(),
...
const createMonitorsBodyValidation = joi.array().items(createMonitorBodyValidation.keys({
