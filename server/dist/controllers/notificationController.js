...
class NotificationController extends BaseController {
    ...
    testNotification = this.asyncHandler(async (req, res) => {
    ...
    }, SERVICE_NAME, "createNotification");
    getNotificationsByTeamId = this.asyncHandler(async (req, res) => {
        const teamId = req?.user?.teamId;
        if (!teamId) {
            throw this.errorService.createBadRequestError("Team ID is required");
        }
    ...
    }, SERVICE_NAME, "getNotificationsByTeamId");
    ...
    }, SERVICE_NAME, "deleteNotification");
    ...
    editNotification = this.asyncHandler(async (req, res) => {
        await createNotificationBodyValidation.validateAsync(req.body, {
            abortEarly: false,
        });
        const teamId = req?.user?.teamId;
    ...
    }, SERVICE_NAME, "editNotification");
    testAllNotifications = this.asyncHandler(async (req, res) => {
        ...
        if (notifications.length === 0)
            throw this.errorService.createBadRequestError("No notifications");
        const result = await this.notificationService.testAllNotifications(notifications);
        if (!result)
            throw this.errorService.createServerError("Failed to send all notifications");
        return res.success({
            msg: "All notifications sent successfully",
        });
    }, SERVICE_NAME, "testAllNotifications");
}
