...
class JobQueueController extends BaseController {
    ...
    }, SERVICE_NAME, "getMetrics");
    getJobs = this.asyncHandler(async (req, res) => {
        const jobs = await this.jobQueue.getJobs();
        return res.success({
            msg: this.stringService.queueGetJobs,
            data: jobs,
        });
    }, SERVICE_NAME, "getJobs");
    getAllMetrics = this.asyncHandler(async (req, res) => {
        const jobs = await this.jobQueue.getJobs();
        const metrics = await this.jobQueue.getMetrics();
        return res.success({
