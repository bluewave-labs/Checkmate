...
import type { MonitorStatus } from "@/db/models/monitors/Monitor.js";

const buildStats = (
	overrides: Partial<IMonitorStats & { save: jest.Mock<any, any> }> = {}
): IMonitorStats => {
	const saveMock =
		overrides.save ||
		jest.fn().mockImplementation(async function (this: unknown) {
			return this;
		});
	return {
		monitorId: {} as any,
...
const buildMonitor = (
	overrides: Partial<IMonitor> & { status?: MonitorStatus }
) => {
	const monitor: any = {
	...
	return {
		...
		saveMock: monitor.save as jest.Mock,
	};
};
...
describe("StatusService.updateMonitorStatus", () => {
	...
	it("initializing monitor adopts first status", async () => {
		const { monitor, saveMock } = buildMonitor({
			status: "initializing" as MonitorStatus,
			latestChecks: [],
		});
		const statusResponse = buildStatusResponse({ status: "up", responseTime: 120 });

		const [savedMonitor, statusChanged] = await service.updateMonitorStatus(
			monitor,
			statusResponse
		);
	...
	it("returns early when not enough checks accumulated", async () => {
		...
		const [, statusChanged] = await service.updateMonitorStatus(
			monitor,
			statusResponse
		);
	...
	it("updates status when last n checks differ", async () => {
		...
		expect(monitor.latestChecks.slice(-2).every((c) => c.status === "down")).toBe(
			true
		);
	});
	...
	it("keeps only latest 25 checks", async () => {
		...
		expect(
			monitor.latestChecks?.some((check) => check.responseTime === 0)
		).toBe(false);
		const lastCheck =
			monitor.latestChecks?.[monitor.latestChecks.length - 1] ?? null;
		expect(lastCheck?.responseTime).toBe(200);
	});

	it("initializes latestChecks when missing", async () => {
		const { monitor, saveMock } = buildMonitor({});
		delete (monitor as any).latestChecks;
...
describe("StatusService.updateMonitorStats", () => {
	const service = new StatusService();
	let findOneSpy: jest.SpiedFunction<typeof MonitorStats.findOne>;
	...
	it("throws ApiError when monitor stats are missing", async () => {
		...
		await expect(
			service.updateMonitorStats(monitor, statusResponse, false)
		).rejects.toBeInstanceOf(ApiError);
		await expect(
			service.updateMonitorStats(monitor, statusResponse, false)
		).rejects.toMatchObject({ message: "MonitorStats not found", status: 500 });
