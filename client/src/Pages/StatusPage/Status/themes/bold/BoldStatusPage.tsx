...
export const BoldStatusPage = ({ statusPage, monitors }: Props) => {
	...
	// Bold uses a dedicated check-disc treatment next to the title; its
	// icon is scaled differently but the semantic resolution is shared.
		...
		<Box className={`${PREFIX}-page`}>
			...
			>
				<Box className={`${PREFIX}-brand`}>
					{logoSrc ? (
						...
						<Box className={`${PREFIX}-logo-conic`}>
			...
			<Box className={`${PREFIX}-hero ${PREFIX}-tone-${overall.tone}`}>
				<h1 className={`${PREFIX}-hero-title`}>
					<span className={`${PREFIX}-hero-check`}>{overall.icon}</span>
					{overall.message}
				</h1>
