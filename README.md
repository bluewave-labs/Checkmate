...
**Container alerts.** Two switches on the monitor form, **Container stopped or missing** and **Health check failing**, send a notification to the monitor's channels when an individual container changes. A stop alerts when the container exits with a non-zero code; containers that exit with code 0 are treated as finished jobs and stay silent. A container that disappears from the host alerts after it has been absent for two consecutive checks, so a container recreated by a deploy between two checks does not. Recoveries send a matching notification. Alerts from one check are batched into a single message, and recoveries into another. After the host itself is unreachable, the next successful check starts from a clean slate: containers that did not come back are recorded as stopped but not alerted. Turning both switches off clears the remembered container states, so re-enabling them later starts clean too. Capture does not report exit codes, so on Capture hosts every stop alerts. On the host page, a container that has alerted or is currently unhealthy is marked next to its name.
